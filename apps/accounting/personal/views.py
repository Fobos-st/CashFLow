...
from .models import PersonalAccount, PersonalTransaction
...
def redirect_account(request):
    ...
    if request.user.is_authenticated:
        user_account = PersonalAccount.objects.filter(user=request.user).values_list('id', flat=True).first()
...
class AccoutingView(View, LoginRequiredMixin):
    ...
    def get_context(self, user:CustomUser, account_id:int, request, get_method=True):
        paginator = Paginator(PersonalTransaction.objects.filter(account=account_id), 20)
        ...
        if get_method:
            return {'form': TransactionCreateForm(user=user), 'account': PersonalAccount.objects.get(pk=account_id),
        ...
        return (TransactionCreateForm(user=user, data=request.POST),
                PersonalAccount.objects.get(pk=account_id),
